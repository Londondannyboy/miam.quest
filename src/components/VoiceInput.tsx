...
function VoiceButton({ onMessage, userName, userId, userEmail }: VoiceInputProps) {
  ...
  // Build system prompt with user context (matches lost.london-v2 pattern)
...
- Be helpful and accurate`;
  ...
  const handleToggle = useCallback(async () => {
    if (status.value === "connected") {
      ...
      try {
        ...
        // Trigger greeting after connection (matches lost.london-v2)
        setTimeout(() => {
  ...
  }, [connect, disconnect, status.value, sendUserInput, userId, userName, userEmail]);
